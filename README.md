...
flowchart TD
    ...
    subgraph REVIEW_PH["レビュー"]
        RG["agent:review-general<br/>① GeneralReviewer<br/>バグ・セキュリティ・性能"]
        RS["agent:review-semantic<br/>② SemanticReviewer<br/>設計規約・影響範囲"]
        RG -->|"Passed"| RS
    end
...
| **`agent:review-general`**  | **一般レビュー (ReviewGeneralAgent)**      | PRに対し、一般的なバグやセキュリティ脆弱性、パフォーマンス問題を検証する。合格すると `agent:review-semantic` へ遷移する。                                                              |
| **`agent:review-semantic`** | **設計規約レビュー (ReviewSemanticAgent)** | PRに対し、設計原則やディレクトリ構造ルールへの適合度、影響範囲を検証する。合格すると `agent:qa` へ遷移する。                                                                           |
...
│   └── runner/                # パイプライン実行エンジン
│       ├── bin/               # CLI・実行用エントリーポイント（run.ts, create-labels.ts）
│       ├── core/              # デーモンのコアエンジン部（crawler.ts, supervisor.ts）
│       ├── tasks/             # タスク実行制御と個別エージェント処理（pool.ts, cli.ts, issue.ts, pr.ts, qa.ts）
│       ├── tui/               # opentuiによるモニタリングダッシュボード（index.ts, widgets.ts）
│       └── workspace/         # 隔離された作業ディレクトリ(git worktree)の構築管理（index.ts）
...
起動すると、TTY環境であれば自動的に TUI ダッシュボードが起動し、各エージェントの進行状況やログが可視化される。TUIを無効にしたい場合は、`--no-tui` オプションを指定する。

```bash
bun run dev:runner -- --repo-map-dir ./repo-map/production
```

単発で1サイクルのみ実行してテストしたい場合は `--once` または `-o` フラグを追加する（TUIは起動しません）

```bash
bun run dev:runner --once -- --repo-map-dir ./repo-map/sandbox
```
