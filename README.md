...
├── pnpm-lock.yaml
├── tsconfig.json
├── repo-map/                  # 対象リポジトリごとの設定 (config.yaml) および構造マップ定義 (*.md)
│   ├── sandbox/               # サンドボックス検証用
│   └── production/            # 本番運用環境用
├── src/
│   ├── core/                  # 共通ユーティリティ（設定ロード、ロガー、安全なコマンド実行 runCommand など）
│   ├── agents/                # 各自律エージェントのプロンプト構築定義 (spec, dev, dev-pr, review, qa)
│   └── runner/                # パイプライン実行エンジン（定期巡回 crawler と異常監視 supervisor）
