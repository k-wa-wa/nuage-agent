...
const QA_PRINCIPLES = `## 効率的な行動原則（重要）
...
export class QAAgent implements Agent {
  ...
  /**
   * @what QAエージェント用の検証項目プロンプトを生成します。
   * @why buildPromptの行数制限（max-lines-per-function）を回避し、かつJSDoc要件（@what/@why）を満たすため。
   */
  private getVerificationItems(prNumber: number): string {
    return `## 検証項目
1. **最新状態の統合確認**:
...
4. **完了基準チェックリストの確認と自己修復（最重要）**:
   PRの概要欄（Body）および紐づく親Issueの概要欄を 'gh pr view ${prNumber}' や 'gh issue view <Issue番号>' で取得し、記載されているすべての完了基準（チェックリストの '- [ ]' 項目）にチェック（'- [x]'）が入っていることを確認する。
   **【自己修復ルール】** テストやビルドは正常に合格しているにもかかわらず、単に未チェックの項目（'- [ ]'）が残っている場合は、開発フェーズに差し戻すのではなく、QAエージェント自身がすべての完了基準を '- [x]' に更新（自己修復）した上で、そのままマージ（合格）処理へ進むこと。
   更新の際、長文によるシェルエスケープのエラーを防ぐため、必ず一時ファイルを用いてPRおよびIssueの概要欄を更新し、直後に削除してください。
   コマンド例: 'echo "[すべての完了基準を- [x]にした本文]" > pr_body.md && gh pr edit ${prNumber} --body-file pr_body.md && rm pr_body.md'
...
${this.getVerificationItems(pr.number)}
...
3. **検証結果の処理**
   - **検証に合格した場合（Passed）**:
     ${successBranchAction}
   - **検証に合格しなかった場合（Failed）**:
     失敗したテスト、競合内容、検出された脆弱性、または完了基準チェックリストに未チェック項目（'- [ ]'）が残っていること（※自己修復が不可能な実装不備の場合）などの不合格理由をPRコメントとして投稿し、ラベルを開発フェーズに戻す。
     コマンド:
       - 不合格コメント投稿: 「gh pr comment ${pr.number} --body "[検証失敗の詳細、または未完了の完了基準（- [ ] の項目）とその原因]"」
