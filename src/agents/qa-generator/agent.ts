...
以下のリポジトリ構成マップとコードベースの構成を参考に、テストやLintの拡充・改善案を1つだけ考えてください。
...
## 任務
リポジトリのソースコード、テスト構成（テストフレームワーク、テストファイル等）、あるいはLint設定を分析し、**極力小さく、すぐに実装・マージが可能なQA改善課題（例：特定の関数に対するテストケースの追加、エッジケースの検証、不足している型アサーションやLintルールの有効化）**を1つだけ見つけ出してください。

見つかった改善点について、GitHub上に新しいIssueを起票してください。

## 起票に関するルール
1. **既存Issueの重複チェック**:
   既に同じ内容、あるいは類似するQA改善Issueが立っていないか、適宜確認してください。
2. **Issueタイトル**:
   必ず \`${this.prefix} \` というプレフィックスをタイトルに付けてください。
   例: \`${this.prefix} math.ts の sum 関数に対するマイナス値のテストケース追加\`
3. **Issue本文**:
   タイトルだけでなく、課題の背景、具体的な実装箇所のファイルパス、および期待されるテスト/改善コードの満たすべき要件（Acceptance Criteria）をMarkdownで記述してください。
4. **ラベルの付与**:
   新しく作成するIssueには、必ず \`agent:spec\` ラベルを付与してください。
