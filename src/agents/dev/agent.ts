...
3. **Pull Requestの作成とラベル変更**
   ...
   - PRのラベル: 「agent:review-general」を付与する。
     コマンド: 「gh pr create --title "feat: #${issue.number} ${issue.title}" --body "Closes #${issue.number}" --label "agent:review-general"」
...
3. **修正内容のプッシュとラベル変更**
   ...
   プッシュ完了後、PRのラベルから「agent:dev」を削除し、再度「agent:review-general」を付与してレビューを依頼する。
   コマンド:
