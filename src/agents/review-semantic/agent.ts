...
export class ReviewSemanticAgent implements Agent {
  ...
  readonly label = 'agent:review-semantic';
...
1. **修正が必要な場合 (Failed)**
   ...
   - ラベル変更: 「gh issue edit ${pr.number} --add-label "agent:dev" --remove-label "agent:review-semantic"」

2. **問題ない場合 (Passed)**
   すべてのチェックに合格した場合、PRに合格判定のレビューコメントを投稿し、ラベルを次の検証フェーズ（agent:qa）に進める。
   ...
   - ラベル変更: 「gh issue edit ${pr.number} --add-label "agent:qa" --remove-label "agent:review-semantic"」
`;
  }
}
